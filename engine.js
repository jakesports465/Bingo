...
function safeInitNewFields(){
  ...
  // so calcOfflineProgress will skip (elapsed from 0 = huge = capped, but we check below)
...
function showCharScreen(){
  try{
    ...
    // Clear broken save and start fresh
...
function addHeat(amount){
  G.heat=Math.min(100,(G.heat||0)+amount);
  updateHeatBar();
...
function payBail(){
  const bailCost=(G.jailTimer||60)*500*Math.max(1,Math.floor(G.level/10));
...
function getEventMult(type,base){
  ...
  if(type==='heatMult'&&activeEventObj.effect==='heatDouble')return base*2;
  if(type==='heatMult'&&activeEventObj.effect==='noRobHeat')return 0;
  if(type==='cashMult'&&activeEventObj.effect==='cashBonus')return base*1.5;
  if(type==='cashMult'&&activeEventObj.effect==='goldenHour')return base*2;
  if(type==='xpMult'&&(activeEventObj.effect==='heatDouble'||activeEventObj.effect==='fightXP2x'||activeEventObj.effect==='goldenHour'))return base*2;
  if(type==='lootMult'&&(activeEventObj.effect==='lootTriple'||activeEventObj.effect==='lootBoost'))return base*(activeEventObj.effect==='lootTriple'?3:1.25);
  return base;
}
...
function applyNotorietyBonuses(){
  ...
  const n=G.notoriety||0;
  if(n>=100)G.cashMult=Math.max(G.cashMult||1,1.05);
  if(n>=300){G.attack=Math.max(G.attack,G.baseAttack+Math.floor(G.attack*.10));}
  if(n>=1000)G.xpMult=Math.max(G.xpMult,G.xpMult*1.20);
}
function gainXP(amt){
  checkAchievements();
  G.xp+=Math.round(amt);
...
function levelUp(){
  ...
  G.xpToNext=Math.floor(60*G.level + 1.5*Math.pow(G.level,1.65));
  ...
  G.maxStamina=Math.floor(20+G.level*.3);
  G.stamina=Math.min(G.maxStamina,G.stamina+Math.floor(G.maxStamina*.2));
  G.maxHealth=Math.floor(100+G.level*5);G.health=G.maxHealth;
  ...
  G.attack=G.baseAttack+G.pBonuses.attackBonus+getSkillBonus('atk')+achAtk;
  G.defense=G.baseDef+G.pBonuses.defBonus+getSkillBonus('def')+achDef;
...
function updateAll(){
  ...
  updateHeatBar();
  // Apply heat cap from dark web
  if(G.heatCap !== undefined) G.heat = Math.min(G.heatCap, G.heat||0);
  // Notoriety perks applied passively
  applyNotorietyPerks();
  updateHeatBar();
...
function startLoops(){
  ...
  setInterval(()=>{
    let ch=false;
    if(G.energy<G.maxEnergy){G.energy=Math.min(G.maxEnergy,G.energy+r.e);ch=true;}
    if(G.stamina<G.maxStamina){G.stamina=Math.min(G.maxStamina,G.stamina+r.s);ch=true;}
  ...
  setInterval(()=>{
    if((G.heat||0)>0){
      ...
      if((G.notoriety||0)>=1500)coolRate*=2;
      coolHeat(coolRate);
    }
  },10000);
