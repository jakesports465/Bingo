...
function buildJobs(c){
  ...
  for(const tier of cityJobs){
    html+=`<div class="tier-hdr">▸ ${tier.tier}</div>`;
    for(const j of tier.jobs){
      const dis=G.energy<j.e;
      ...
      html+=`<div class="jcard ${dis?'jdis':''} ${mastered?'jmas':''}" id="jc-${j.id}">
        <div>
          <div class="jn">${mastered?'★ ':''} ${j.n} ${tBadge}</div>
          <div class="jd">${j.d}</div>
          <div class="jreq">⚡ ${j.e} energy${j.li?` &nbsp;🎁 Loot possible`:''}</div>
        ...
        <div class="jr">
          ...
          ${j.li?`<span class="jrl">🎁 ${(j.lc*100+G.lootBonus).toFixed(0)}% drop</span>`:''}
        </div>
...
function doJob(id){
  ...
  if(G.playerClass==='Mogul')cashMult*=1.15;
  cashMult*=G.pBonuses.cashMult;
  const cash=Math.floor((j.c+Math.floor(Math.random()*j.c*.2))*cashMult);
  G.cash+=cash;G.totalEarned+=cash;
  const xp=Math.round(j.x*G.xpMult*G.pBonuses.xpMult);
  gainXP(xp);
  G.jobsDone++;
  ...
  if(j.li&&Math.random()<lc){addLoot(j.li);}
...
const ENEMIES = [
  // ── STREET THUGS ──
  {n:'Danny the Rat',rank:'Street Thug',atk:6,def:4,ml:1,r:105,xr:6,kc:.08,tier:0,loot:null},
  {n:'Skinny Pete',rank:'Pickpocket',atk:4,def:3,ml:1,r:70,xr:5,kc:.06,tier:0,loot:null},
  {n:'Alley Cat Jones',rank:'Mugger',atk:8,def:5,ml:1,r:140,xr:7,kc:.09,tier:0,loot:'brass_knuckles'},
  {n:'Knuckles McGee',rank:'Corner Bookie',atk:10,def:7,ml:2,r:196,xr:10,kc:.10,tier:0,loot:'brass_knuckles'},
  {n:'Dumpster Dave',rank:'Fence',atk:12,def:8,ml:3,r:244,xr:12,kc:.10,tier:0,loot:'fake_rolex'},
  {n:'Frankie Two Shoes',rank:'Numbers Runner',atk:14,def:10,ml:4,r:315,xr:15,kc:.12,tier:0,loot:null},
  {n:'Big Sal Romano',rank:'Enforcer',atk:20,def:15,ml:7,r:489,xr:22,kc:.14,tier:0,loot:'switchblade'},
  {n:'Tommy the Torch',rank:'Arsonist',atk:16,def:8,ml:5,r:385,xr:17,kc:.11,tier:0,loot:'dynamite'},
  {n:'Needles Moretti',rank:'Loan Shark',atk:24,def:18,ml:9,r:630,xr:27,kc:.13,tier:0,loot:'gold_chain'},
  {n:'Vinnie the Knife',rank:'Soldier',atk:28,def:22,ml:10,r:770,xr:30,kc:.15,tier:0,loot:'switchblade'},
  // ── MADE MEN ──
  {n:'Marco the Butcher',rank:'Capo',atk:40,def:32,ml:15,r:1260,xr:45,kc:.17,tier:1,loot:'pistol'},
  {n:'Don Eduardo Santini',rank:'Underboss',atk:55,def:44,ml:20,r:2100,xr:67,kc:.18,tier:1,loot:'gold_watch'},
  {n:'The Sicilian',rank:'Consigliere',atk:75,def:60,ml:28,r:3500,xr:97,kc:.19,tier:1,loot:'silenced_pistol'},
  {n:'Ghost',rank:'Assassin',atk:95,def:78,ml:35,r:5600,xr:135,kc:.20,tier:1,loot:'assassination_kit'},
  {n:'Lucky Lucchese',rank:'Family Boss',atk:68,def:55,ml:25,r:2940,xr:80,kc:.18,tier:1,loot:'don_ring'},
  {n:'Crazy Eyes Colombo',rank:'Hitman',atk:85,def:65,ml:30,r:4550,xr:115,kc:.19,tier:1,loot:'desert_eagle'},
  // ── BOSSES & CAPOS ──
  {n:'Viktor Solokov',rank:'Bratva Commander',atk:120,def:100,ml:45,r:9100,xr:195,kc:.21,tier:2,loot:'ak47'},
  {n:'Chen Wei',rank:'Triad Master',atk:150,def:125,ml:55,r:14000,xr:255,kc:.22,tier:2,loot:'jade_dragon'},
  {n:'El Señor',rank:'Cartel Kingpin',atk:190,def:160,ml:65,r:22400,xr:330,kc:.23,tier:2,loot:'cartel_pistol'},
  {n:'Black Sun Yoshida',rank:'Yakuza Oyabun',atk:240,def:200,ml:80,r:35000,xr:420,kc:.24,tier:2,loot:'katana'},
  {n:'Natasha Volkov',rank:'Red Mafiya Queen',atk:180,def:150,ml:60,r:19600,xr:300,kc:.22,tier:2,loot:'polonium_vial'},
  {n:'Ibrahim Al-Rashid',rank:'Gulf Warlord',atk:210,def:175,ml:70,r:26600,xr:360,kc:.23,tier:2,loot:'gold_bar'},
  // ── INTERNATIONAL ──
  {n:'The Architect',rank:'Shadow Broker',atk:300,def:255,ml:100,r:56000,xr:540,kc:.25,tier:3,loot:'hacking_rig'},
  {n:'Cardinal Leone',rank:'Vatican Insider',atk:370,def:315,ml:125,r:84000,xr:675,kc:.26,tier:3,loot:'papal_signet'},
  {n:'Iron Duke Morozov',rank:'Russian Oligarch',atk:460,def:395,ml:150,r:125999,xr:840,kc:.27,tier:3,loot:'vor_crown'},
  {n:'The Dragon Lady',rank:'Asian Syndicate Chief',atk:570,def:490,ml:180,r:175000,xr:1050,kc:.27,tier:3,loot:'dragon_seal'},
  {n:'Phantom',rank:'International Assassin',atk:700,def:605,ml:210,r:280000,xr:1320,kc:.28,tier:3,loot:'sniper_rifle'},
  {n:'Baron von Krieg',rank:'Arms Dealer Lord',atk:520,def:440,ml:165,r:140000,xr:950,kc:.26,tier:3,loot:'kalashnikov'},
  // ── LEGENDARY ──
  {n:'Lazarus Vane',rank:'Dark Network Controller',atk:880,def:760,ml:250,r:489999,xr:1650,kc:.28,tier:4,loot:'crypto_wallet'},
  {n:'The Eternal Don',rank:'Immortal Crime Lord',atk:1100,def:955,ml:300,r:840000,xr:2100,kc:.29,tier:4,loot:'world_don_crown'},
  {n:'Shadow Emperor Kai',rank:'World Syndicate Boss',atk:1400,def:1210,ml:375,r:1400000,xr:2700,kc:.30,tier:4,loot:'samurai_sword'},
  {n:'The Devil',rank:'Primordial Evil',atk:1800,def:1560,ml:450,r:2100000,xr:3450,kc:.30,tier:4,loot:'golden_ak'},
  {n:'Omega Prime',rank:'Final Adversary',atk:2300,def:1990,ml:550,r:3500000,xr:4500,kc:.31,tier:4,loot:null},
  // ── MYTHIC ──
  {n:'God of Crime',rank:'Transcendent',atk:3000,def:2600,ml:700,r:5600000,xr:6000,kc:.32,tier:5,loot:null},
  {n:'The Last Enemy',rank:'End of Days',atk:5000,def:4300,ml:900,r:14000000,xr:9000,kc:.33,tier:5,loot:null},
  {n:'Il Diavolo',rank:'Hell\'s Don',atk:8000,def:7000,ml:1100,r:21000000,xr:15000,kc:.35,tier:5,loot:null},
  {n:'The Absolute',rank:'Beyond Mortal',atk:15000,def:13000,ml:1300,r:35000000,xr:30000,kc:.38,tier:5,loot:null},
  {n:'Maximum Don',rank:'Level 1500 Challenge',atk:25000,def:22000,ml:1480,r:84000000,xr:75000,kc:.40,tier:5,loot:null},
];
  ...
  for(let ti=0;ti<=5;ti++){
    ...
    for(const e of tierEnemies){
      html+=`<div class="ecard" style="border-left:3px solid ${etColors[e.tier]};">
        ...
        <button class="fb" onclick="doFight('${e.n.replace(/'/g,"\\'")}')">⚔️ ATTACK (1 💪)</button>
      </div>`;
    }
    html+=`</div>`;
  }
...
function doFight(ename){
  if(G.stamina<1){toast('No stamina!','r');return;}
  ...
  if(win){
    ...
    let cash=Math.floor(e.r*(1+Math.random()*.3)*(1+streakBonus/100));
    const xp=Math.round(e.xr*G.xpMult*G.pBonuses.xpMult);
    ...
    gainXP(xp);
    if(Math.random()<e.kc){G.kills++;addLog(`💀 ${e.n} eliminated!`,'sp');}
    ...
    // Loot drop
    if(e.loot&&Math.random()<.12){addLoot(e.loot);}
...
function gangWarAttack(gid,blitz){
  ...
  const gain=power>ePow?Math.floor(15+Math.random()*20)*(blitz?2:1):Math.floor(Math.random()*5);
...
function doContract(name){
  ...
  const diffMult={easy:.85,med:.7,hard:.55,elite:.45}[ct.diff]||.7;
  const win=Math.random()>diffMult;
  if(win){
    let cash=ct.cb+(Math.floor(Math.random()*ct.cb*.25));
...
function buildHitman(c){
  ...
  for(const tier of tiers){
    ...
    for(const t of targets){
      ...
      const realRate=Math.min(.95,t.successRate+notBonus+crewBonus);
      ...
      if(done){
        ...
        html+='<button onclick="doHit(\''+t.id+'\')" style="background:linear-gradient(135deg,#1a0000,#3a0000);border:2px solid var(--crimson);color:var(--bright-gold);font-family:\'Bebas Neue\',sans-serif;font-size:12px;letter-spacing:.1em;padding:7px 12px;cursor:pointer;white-space:nowrap;" '+(G.cash<t.cost||G.stamina<3?'disabled':'')+'>HIT (3💪)</button>';
      } else {
...
function doPrestige(){
  ...
  G.level=1;G.xp=0;G.xpToNext=20;G.skillPoints+=10;
  G.energy=G.maxEnergy;G.stamina=G.maxStamina;G.health=G.maxHealth;
  G.attack=G.baseAttack+G.pBonuses.attackBonus+getSkillBonus('atk');
  G.defense=G.baseDef+G.pBonuses.defBonus+getSkillBonus('def');
...
function doOp(id){
  ...
  addHeat(5+op.tier*3);
  if(p>=3){
    const cash=Math.floor(op.c*(G.cashMult||1));
...
function healPlayer(){
  const cost=Math.floor(G.maxHealth*2);
  if(G.cash<cost){toast(`Heal costs $${cost.toLocaleString()}!`,'r');return;}
  G.cash-=cost;G.health=G.maxHealth;
  addLog(`💊 Healed to full for $${cost.toLocaleString()}`,'good');toast('Fully healed!','g');
  updateAll();save();
}
...
function buildRacing(c){
  ...
  for(let ti=0;ti<RACE_TIER_NAMES.length;ti++){
    ...
    for(const race of races){
      ...
      const onCd=cdLeft>0;
      const winChance=Math.min(0.95,Math.max(0.05,(effectiveSpeed/race.diff)*0.7+0.15));
...
function doHack(hid){
  ...
  if(Math.random()>hack.risk){
    const earn=Math.floor(hack.reward*(G.cashMult||1));
    G.cash+=earn;G.totalEarned+=earn;
...
function doStreetBounty(bid){
  ...
  const repIdx=UW_REP_TIERS.indexOf(repTier);
  const repMult=1+(repIdx>=9?2:repIdx>=8?1:repIdx*.08);
  const earn=Math.floor(b.cash*(G.cashMult||1)*repMult);
  G.cash+=earn;G.totalEarned+=earn;
  G.underworldRep=(G.underworldRep||0)+b.rep;
  if(!G.streetBounties)G.streetBounties={};
  // Cooldown halved at high rep
  const cdMult=repIdx>=5?.5:1;
  G.streetBounties[bid]=Date.now()+b.cd*1000*cdMult;
  gainXP(Math.floor(b.rep/3));
  addNotoriety(Math.floor(b.rep/50));
  addLog(`🌆 ${b.n}: +$${fmtCash(earn)} +${b.rep} rep`,'gold');
