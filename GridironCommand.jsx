...
function T(id) {
  const raw = TEAMS[id] || EXTRA_TEAMS[id] || null;
  const base = raw || { id, name: String(id || "?"), short: String(id || "TBD").replace(/^cfbd-/, "").slice(0, 4).toUpperCase(), colors: ["#555", "#999"] };
  const idn = IDENTITY[id];
  let colors = (idn && idn.colors && idn.colors.length) ? idn.colors : base.colors;
  if (!Array.isArray(colors) || !colors.length) colors = ["#555", "#999"];
  if (colors.length < 2) colors = [colors[0], colors[0]];
  const short = base.short || String(id || "TBD").replace(/^cfbd-/, "").slice(0, 4).toUpperCase();
  const name = base.name || String(id || "Unknown");
  return { ...base, name, short, colors, logos: idn ? idn.logos : base.logos, abbr: (idn && idn.abbr) || base.abbr };
}
/* Safe wrappers — a bad game can never crash a whole screen (V3 hardening). */
function computeJakeRating(g, p) { try { return _raw_computeJakeRating(g, p); } catch (e) { return { score: 5, reasons: [] }; } }
function oldSchool(g) { try { return _raw_oldSchool(g); } catch (e) { return { score: 0, badges: [], reasons: [] }; } }
function rootingFor(g, p) { try { return _raw_rootingFor(g, p); } catch (e) { return { team: null, text: "" }; } }
function jakesTake(g, p) { try { return _raw_jakesTake(g, p); } catch (e) { return null; } }
function specialChips(g) { try { return _raw_specialChips(g); } catch (e) { return []; } }
...
function _raw_computeJakeRating(g, priorities) {
  const reasons = [];
  let s = 2.0;
...
function _raw_jakesTake(g, priorities) {
...
function _raw_rootingFor(g, priorities) {
  const a = g.away, h = g.home;
  ...
  if (dis.length === 2) {
    return { team: null, text: `No dog in this fight — both ${T(a).short} and ${T(h).short} are on your dislike list. Root for chaos, injuries to no one, and a result that muddies their playoff math.` };
  }
  // default: underdog / lower projected rank, else run-heavy
  const ar = RANK[a] || 99, hr = RANK[h] || 99;
  const dog = ar > hr ? a : h;
  return { team: dog, text: `No stake here — lean ${T(dog).short} as the underdog. Chaos helps everyone below the projected favorite.` };
}
...
class CardBoundary extends React.Component {
  constructor(p) { super(p); this.state = { e: 0 }; }
  static getDerivedStateFromError() { return { e: 1 }; }
  render() { if (this.state.e) return <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: 10, fontSize: 11, color: C.dim }}>A game card couldn't render — skipped.</div>; return this.props.children; }
}
function GameCardSafe(props) { return <CardBoundary><GameCard {...props} /></CardBoundary>; }
...
function HomeView({ priorities, onEnterBooth, ...rest }) {
  ...
  return (
    <div style={{ padding: "0 14px 90px" }}>
      ...
      {nextUp.length > 0 && (
        <>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {nextUp.map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
          </div>
        </>
      )}
      ...
      <GameCardSafe g={gotw} priorities={priorities} {...rest} expanded={exp === gotw.id} onToggle={() => setExp(exp === gotw.id ? null : gotw.id)} />

      <SectionTitle icon={Sparkles}>SPECIAL EVENT ON THE RADAR</SectionTitle>
      {specialEvent && <GameCardSafe g={specialEvent} priorities={priorities} {...rest} expanded={exp === specialEvent.id} onToggle={() => setExp(exp === specialEvent.id ? null : specialEvent.id)} />}

      <SectionTitle icon={CalendarDays}>SEASON OPENS — WEEK 1</SectionTitle>
      <div style={{ display: "grid", gap: 8 }}>
        {w1.map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
      </div>
    </div>
  );
}
            ...
            <div key={w} style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {byDay[d][w].sort((a, b) => computeJakeRating(b, priorities).score - computeJakeRating(a, priorities).score)
                  .map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} overrides={overrides} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
              </div>
            </div>
          ))}
        </div>
...
function TeamDetail({ id, priorities, onBack, ...rest }) {
  ...
  return (
    <div style={{ padding: "12px 14px 90px" }}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {games.map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
      </div>
...
function EventsView({ priorities, ...rest }) {
  ...
  return (
    <div style={{ padding: "12px 14px 90px" }}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {events.map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
      </div>
    </div>
  );
}
...
function NationalView({ priorities, mode, ...rest }) {
  ...
  return (
    <div style={{ padding: "0 14px 90px" }}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {visible.map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
      </div>
  ...
  const Block = ({ icon: Icon, label, g, text }) => {
    ...
    return (
      <div style={{ marginBottom: 14 }}>
        ...
        {g && <GameCardSafe g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />}
      </div>
    );
  };
    ...
    <div style={{ padding: "0 14px 90px" }}>
      ...
      {myGames.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {myGames.map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
          </div>
        </div>
      )}
      ...
      {weeknight.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {weeknight.map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
          </div>
        </div>
      )}
    </div>
    ...
    <div style={{ padding: "0 14px 90px" }}>
      ...
      {todayGames.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {todayGames.map((g) => <GameCardSafe key={g.id} g={g} priorities={priorities} {...rest} expanded={exp === g.id} onToggle={() => setExp(exp === g.id ? null : g.id)} />)}
          </div>
        </div>
      )}
...
async function syncCFBD({ key, results, overrides, full, prevLive }) {
  ...
  if (full) {
    if (rankingsRaw && rankingsRaw.length) {
      ...
      if (ap) { pollName = ap.poll; (ap.ranks || []).forEach((r) => { const id = NAME_TO_ID[normName(r.school)] || ("cfbd-" + normName(r.school)); ranks[id] = r.rank; rankedCount++; rankedNames[id] = r.school; }); }
  ...
  Object.keys(rankedNames).forEach((id) => { if (!TEAMS[id] && !liveTeams[id]) liveTeams[id] = { id, name: rankedNames[id], short: shortFromName(rankedNames[id]), conf: "FBS", colors: colorFromName(rankedNames[id]) }; });
