...
// Check for existing session — called on page load to skip login if already authenticated
async function tryRestoreSession() {
  if (!window._sb) return false;
  try {
    const { data } = await window._sb.auth.getSession();
    if (data && data.session && data.session.user) {
      window._sbUser = data.session.user;
      console.log('Session restored for:', window._sbUser.email);
      return true;
    }
  } catch(e) {
